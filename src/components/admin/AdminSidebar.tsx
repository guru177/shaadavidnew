...
export default function AdminSidebar() {
  ...
  const navItems = [
    ...
    { name: 'Legal pages', href: '/admin/legal', icon: (
  ...
  return (
    <div className="relative z-20 flex h-full w-64 shrink-0 flex-col overflow-hidden bg-[#0c1622] text-white shadow-xl">
      <div className="flex h-20 shrink-0 items-center justify-center border-b border-white/10 px-6">
        <div className="relative h-14 w-14 overflow-hidden rounded-full bg-black shadow-inner">
          <Image src="/logo.png" alt="Shaa David's Academy" fill className="object-cover" />
        </div>
      </div>

      <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-3 py-5">
        ...
        <nav className="space-y-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <div className={isActive ? "text-white" : "text-white/40"}>{item.icon}</div>
                {item.name}
              </Link>
            );
          })}
      ...
      <div className="shrink-0 border-t border-white/10 p-3">
        <Link 
          ...
          className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-red-400 transition-colors hover:bg-red-500/10"
        >
