...
export default function Header() {
  ...
  return (
    <>
      {/* Spacer to prevent layout shift since header is now fixed */}
      <div className="w-full h-[72px] md:h-[96px] 2xl:h-[128px] shrink-0 pointer-events-none"></div>

      <header 
        className={`flex items-center justify-between px-5 md:px-8 xl:px-12 2xl:px-16 fixed top-0 left-1/2 -translate-x-1/2 w-full max-w-[1920px] z-[100] transition-all duration-300 ${
          isScrolled 
      ...
      <div className="flex items-center flex-1 relative z-50">
        {/* Logo */}
        <div className="relative w-[130px] md:w-[160px] xl:w-[180px] 2xl:w-[220px] h-8 xl:h-10 2xl:h-12 flex items-center">
          <Image
            src="/logo.svg"
            alt="TestiQA Logo"
            fill
      ...
      <div className="flex items-center justify-end w-auto xl:w-[38%] pr-0 xl:pr-4 gap-4 xl:gap-12 relative z-50">
        {/* Get in touch */}
        <div className={`flex items-center gap-2 xl:gap-3 transition-colors duration-300 ${isScrolled ? 'text-[#29425e]' : 'text-[#29425e] xl:text-white'}`}>
          <span className="font-medium text-[13px] md:text-sm 2xl:text-lg hidden sm:block">Get in touch</span>
          <a href="tel:+917907075923" className={`p-2 md:p-2.5 2xl:p-3.5 rounded-full transition-colors backdrop-blur-sm group ${
            isScrolled 
