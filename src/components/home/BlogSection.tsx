  ...
  return (
    <section className="relative w-full bg-[#FAFAFA] py-[80px] md:py-[100px] px-5 md:px-8 xl:px-12 2xl:px-16 max-w-[1920px] mx-auto overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 2xl:gap-16">
        ...
        <div className="col-span-1 lg:col-span-6 flex flex-col group cursor-pointer bg-white rounded-[24px] 2xl:rounded-[30px] shadow-sm hover:shadow-lg transition-shadow border border-gray-100 overflow-hidden">
          ...
          <div className="p-6 md:p-8 2xl:p-10 flex flex-col flex-1">
            <h3 className="text-3xl md:text-4xl 2xl:text-4xl font-malayalam font-extrabold text-[#29425e] mb-6 group-hover:opacity-80 transition-colors leading-[1.4]">
              ഇംഗ്ലീഷ് സംസാരിക്കാൻ പേടിയാണോ? ഈ 5 കാര്യങ്ങൾ ശ്രദ്ധിച്ചാൽ മതി
            </h3>
            <div className="flex items-center gap-4 text-sm md:text-base 2xl:text-lg text-gray-500 font-medium flex-wrap mt-auto">
              <div className="flex items-center gap-2">
                <img loading="lazy" src="https://ui-avatars.com/api/?name=Shaa+David&background=29425e&color=fff" alt="Author" className="w-8 h-8 2xl:w-10 2xl:h-10 rounded-full object-cover" />
                <span className="text-[#111] font-bold">Shaa David</span>
              </div>
              <span className="hidden sm:block text-gray-300">|</span>
              <span className="px-3 py-1 bg-[#29425e]/10 text-[#29425e] rounded-md font-semibold">Tips & Tricks</span>
              <span className="hidden sm:block text-gray-300">|</span>
              <span>Oct 11, 2024 • 8 min</span>
            </div>
          </div>
        </div>

        {/* Right: Trending Block & Small Posts (col-span-5) */}
        <div className="col-span-1 lg:col-span-6 flex flex-col gap-8 2xl:gap-10">

          {/* Trending Block */}
          <div className="w-full rounded-[20px] 2xl:rounded-[30px] bg-[linear-gradient(110deg,#29425e_0%,#395c80_30%,#0c1622_50%,#395c80_70%,#29425e_100%)] bg-[length:200%_auto] animate-shimmer p-8 sm:p-10 2xl:p-12 relative overflow-hidden shadow-lg flex items-center justify-between min-h-[160px] 2xl:min-h-[200px] group cursor-pointer">
            {/* Title */}
            <h3 className="text-2xl md:text-3xl 2xl:text-4xl font-bold text-white relative z-10 leading-[1.3] w-[60%] group-hover:scale-105 transition-transform duration-500 origin-left">
            ...
            <div className="absolute right-0 top-0 w-[50%] h-full flex flex-col items-end justify-center pointer-events-none opacity-90">
              <div className="w-20 h-20 2xl:w-28 2xl:h-28 bg-white/10 backdrop-blur-md rounded-2xl rotate-12 -translate-y-6 translate-x-4 flex items-center justify-center transform group-hover:-rotate-12 transition-transform duration-700 shadow-[0_10px_30px_rgba(0,0,0,0.1)]">
                <div className="flex gap-1.5">
                  <div className="w-1.5 h-6 bg-white/70 rounded-full animate-pulse"></div>
                  <div className="w-1.5 h-10 bg-white/70 rounded-full animate-pulse delay-75"></div>
                  <div className="w-1.5 h-8 bg-white/70 rounded-full animate-pulse delay-150"></div>
                  <div className="w-1.5 h-5 bg-white/70 rounded-full animate-pulse delay-300"></div>
                </div>
              </div>
              <div className="absolute bottom-[-10px] right-[70px] w-14 h-14 2xl:w-20 2xl:h-20 bg-black/10 backdrop-blur-md rounded-2xl -rotate-12 flex items-center justify-center border border-white/5 shadow-xl">
          ...
          <div className="flex flex-col gap-6 2xl:gap-8">
            {[
              ...
              <div key={idx} className="flex gap-4 sm:gap-6 group cursor-pointer bg-white rounded-[20px] 2xl:rounded-[24px] p-4 2xl:p-5 shadow-sm hover:shadow-md transition-shadow border border-gray-100 items-center">
                {/* Thumbnail */}
                <div className="w-[100px] h-[80px] sm:w-[130px] sm:h-[90px] 2xl:w-[160px] 2xl:h-[110px] shrink-0 rounded-[12px] 2xl:rounded-[16px] overflow-hidden relative shadow-sm">
                ...
                <div className="flex flex-col justify-center flex-1 h-full py-1">
                  <h4 className="text-base sm:text-lg 2xl:text-xl font-malayalam font-bold text-[#29425e] leading-[1.4] mb-3 group-hover:opacity-80 transition-colors line-clamp-2">
                    {post.title}
                  </h4>
                  <div className="flex items-center gap-2 sm:gap-3 text-[11px] sm:text-xs 2xl:text-sm text-gray-500 font-medium flex-wrap mt-auto">
                    <div className="flex items-center gap-1.5">
                      <div className="w-5 h-5 2xl:w-6 2xl:h-6 rounded-full bg-[#29425e] text-white flex items-center justify-center text-[9px] 2xl:text-[10px] font-bold">SD</div>
                      <span className="text-[#111] font-bold">Shaa David</span>
                    </div>
                    <span className="hidden sm:block text-gray-300">|</span>
                    <span className="px-2 py-0.5 bg-[#29425e]/10 text-[#29425e] rounded font-semibold whitespace-nowrap">{post.tag}</span>
                    <span className="hidden sm:block text-gray-300">|</span>
                    <span>{post.date}</span>
                  </div>
                </div>
