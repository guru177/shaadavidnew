  ...
  return (
    <section className="relative w-full bg-[#FAFAFA] py-[80px] md:py-[100px] px-5 md:px-8 xl:px-12 2xl:px-16 max-w-[1920px] mx-auto overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 2xl:gap-16">
        ...
        <Link href={`/blogs/${featuredPost.slug}`} className="col-span-1 lg:col-span-6 flex flex-col group cursor-pointer bg-white rounded-[28px] 2xl:rounded-[30px] shadow-sm hover:shadow-lg transition-shadow border border-gray-100 overflow-hidden">
          ...
          <div className="p-5 sm:p-8 2xl:p-10 flex flex-col flex-1">
            ...
            <div className="flex items-center gap-3 sm:gap-4 text-xs sm:text-base 2xl:text-lg text-gray-500 font-medium flex-wrap mt-auto">
              <div className="flex items-center gap-2">
                <img loading="lazy" src={`https://ui-avatars.com/api/?name=${featuredPost.author.replace(' ', '+')}&background=29425e&color=fff`} alt="Author" className="w-7 h-7 sm:w-8 sm:h-8 2xl:w-10 2xl:h-10 rounded-full object-cover" />
                <span className="text-[#111] font-bold">{featuredPost.author}</span>
              </div>
              <span className="hidden xs:block text-gray-300">|</span>
              <span className="px-2.5 py-1 bg-[#29425e]/10 text-[#29425e] rounded-md font-semibold text-[10px] sm:text-sm">{featuredPost.category}</span>
              <span className="hidden sm:block text-gray-300">|</span>
              <span className="text-[10px] sm:text-sm">{featuredPost.date} • {featuredPost.readTime}</span>
            </div>
          </div>
        </Link>
        ...
        <div className="col-span-1 lg:col-span-6 flex flex-col gap-6 sm:gap-8 2xl:gap-10">
          ...
          <div className="flex flex-col gap-4 sm:gap-6 2xl:gap-8">
            {smallPosts.map((post: any) => (
              <Link key={post.id} href={`/blogs/${post.slug}`} className="flex gap-3 sm:gap-6 group cursor-pointer bg-white rounded-[20px] 2xl:rounded-[24px] p-3 sm:p-4 2xl:p-5 shadow-sm hover:shadow-md transition-shadow border border-gray-100 items-center">
                {/* Thumbnail */}
                <div className="w-[80px] h-[65px] sm:w-[130px] sm:h-[90px] 2xl:w-[160px] 2xl:h-[110px] shrink-0 rounded-[10px] sm:rounded-[12px] 2xl:rounded-[16px] overflow-hidden relative shadow-sm">
                  <img loading="lazy" src={post.image} alt={post.title} className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700" />
                </div>
                {/* Details */}
                <div className="flex flex-col justify-center flex-1 h-full py-0.5 sm:py-1">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 text-[9px] sm:text-xs 2xl:text-sm text-gray-500 font-medium flex-wrap mt-auto">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[#111] font-bold text-[9px] sm:text-xs">{post.author}</span>
                    </div>
                    <span className="hidden xs:block text-gray-300">|</span>
                    <span className="px-1.5 py-0.5 bg-[#29425e]/10 text-[#29425e] rounded font-semibold whitespace-nowrap text-[8px] sm:text-[10px]">{post.category}</span>
                  </div>
                </div>
              </Link>
            ))}
