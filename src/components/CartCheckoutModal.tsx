  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const orderRes = await fetch("/api/razorpay/create-order", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ items, address, subtotal: cartTotal }),
      });
      const orderData = await orderRes.json();
      if (!orderRes.ok) throw new Error(orderData.error || "Failed to create payment order");

      const verify = async (payment: Record<string, string>) => {
      ...
      const rzp = new window.Razorpay({
        ...
        image: cart[0]?.image || "/product.webp",
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex border-b border-gray-100 bg-white shadow-[0_2px_10px_rgba(0,0,0,0.02)] z-10 relative">
          {["വിലാസം", "പേയ്‌മെന്റ്", "പൂർത്തിയായി"].map((lbl, idx) => {
            ...
            return (
              <div
                key={lbl}
                className={`flex-1 text-center py-3.5 text-sm font-medium border-b-[3px] transition-all duration-300 ${
                  isActive
        ...
        >
          {step === 1 && (
            <form id="cart-checkout-form" onSubmit={handleAddressSubmit} className="space-y-4">
              <h3 className="text-lg font-medium text-[#0c1622] mb-4">ഡെലിവറി വിലാസം ചേർക്കുക</h3>

              <div className="rounded-xl border border-gray-200 bg-white p-4 text-sm space-y-2 mb-2">
                {cart.map((c) => (
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-gray-300 rounded-xl px-3 py-2.5 outline-none focus:border-[#395c80] focus:ring-1 focus:ring-[#395c80] transition-all bg-white shadow-sm resize-none"
                />
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6 flex flex-col items-center py-2">
              ...
              <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
                <div>
                  <div className="font-semibold text-[#0c1622]">{summaryLabel}</div>
          ...
          {step === 3 && (
            <div className="text-center py-1 flex flex-col items-center">
              ...
              <p className="text-gray-500 text-sm max-w-sm mx-auto mb-4 leading-relaxed">
                നന്ദി, <strong className="text-[#0c1622]">{address.name}</strong>. നിങ്ങളുടെ ഓർഡർ സ്ഥിരീകരിച്ചു.
              </p>
              ...
              <div className="w-full max-w-sm mx-auto">
                <div className="bg-white border border-gray-100/50 rounded-2xl p-4 text-left shadow-lg shadow-gray-200/40">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[14px]">
                    ...
                    <div>
                      <div className="text-[11px] text-gray-400 font-bold uppercase tracking-wider mb-0.5">തുക</div>
            ...
            <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
              ...
              {step === 1 ? (
                <button
                  ...
                  className="flex-[2] sm:flex-none px-4 sm:px-8 py-2.5 bg-[linear-gradient(110deg,#29425e_0%,#395c80_100%)] hover:shadow-lg text-white rounded-xl font-medium shadow-sm transition-all text-sm sm:text-base whitespace-nowrap disabled:opacity-50"
                >
                  തുടരുക
                </button>
              ) : (
